...
export function MemoEmojiBar({
  ...
  return (
    <div className="memo-emoji-bar-wrap">
      ...
      {onInsertCircledNumber ? (
        <div
          className="memo-emoji-bar memo-emoji-bar--circled"
          role="toolbar"
          aria-label="번호"
        >
          {MEMO_CIRCLED_NUMBERS.map((item) => (
            <button
              key={item.char}
              type="button"
              className="memo-emoji-bar-btn memo-emoji-bar-btn--circled"
              aria-label={`${item.label}번 삽입`}
              disabled={disabled}
              onMouseDown={(e) => e.preventDefault()}
              onClick={() => onInsertCircledNumber(item.char)}
            >
              {item.char}
            </button>
          ))}
