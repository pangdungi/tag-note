-- 009 실행 중 INSERT 23505로 중단된 경우 — 이 파일만 SQL Editor에서 실행
-- (테이블·컬럼은 이미 있을 수 있음 — 모두 IF NOT EXISTS / idempotent)
...
-- PostgREST 스키마 캐시 갱신 (Supabase API가 sources 테이블을 인식하게)
